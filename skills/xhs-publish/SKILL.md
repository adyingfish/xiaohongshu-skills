...
- 段落之间使用双换行分隔。
- 简体中文，语言自然。
- 话题与正文分开准备，通过 `--tags "标签1" "标签2"` 传入，不带 `#`，去重后最多 10 个。正文文件不追加 `#话题`。
- 用户提供的末尾独立话题行应移到 `--tags`；正文中的普通井号、Markdown 标题或引用不改写。
- 话题必须通过编辑器联想列表选择。输入 `#话题` 文本不等于关联话题；联想失败时停止，不能补空格后继续发布。预览时核对选中的话题名称。
...
将标题和正文分别写入 UTF-8 文本文件，不在命令行中内联标题或正文；话题名称通过 `--tags` 单独传入。
...
注意：长文话题在此步骤通过 `next-step --tags` 添加，不写进长文排版正文。发布页的描述编辑器是独立的，需要单独填入内容。如果描述超过 1000 字，脚本会自动截断到 800 字。
