...
description: |
  小红书自动化技能集合。支持认证登录、内容发布、搜索发现、社交互动（含私信读取、待回复、通知和关注）、复合运营。
  当用户要求操作小红书（发布、搜索、评论、登录、分析、点赞、收藏、私信、草稿管理、内容导出）时触发。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
2. **内容发布**（"发布 / 发帖 / 上传图文 / 上传视频 / 管理自己的笔记与草稿"）→ 执行 `xhs-publish` 技能。
3. **搜索发现**（"搜索笔记 / 查看详情 / 浏览首页 / 查看用户 / 查收藏与专辑 / 解析链接 / 导出内容"）→ 执行 `xhs-explore` 技能。
4. **社交互动**（"评论 / 回复 / 点赞 / 收藏 / 私信历史与待回复 / 通知 / 关注状态"）→ 执行 `xhs-interact` 技能。
5. **复合运营**（"竞品分析 / 热点追踪 / 批量互动 / 一键创作"）→ 执行 `xhs-content-ops` 技能。

需要日常管理新命令时，按上述职责读取子技能及 [日常使用与运营管理](docs/daily-operations.md)。结果标记不完整时，必须说明读取范围，不能把部分列表当全量。
